...
def register():
    ...
    if form.validate_on_submit():
        ...
        if User.query.filter_by(email=email).first():
            ...
            salted_and_hashed_password = generate_password_hash(password)
            user = User(email=email, password=salted_and_hashed_password, name=name)
            db.session.add(user)
            db.session.commit()
            login_user(user)
...
def login():
    ...
    if form.validate_on_submit():
        ...
        elif not check_password_hash(fetched_user.password, password):
